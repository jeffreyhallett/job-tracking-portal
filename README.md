...
- **Session tokens** are stateless: an HMAC over (user id, issued-at) keyed on *that user's own password hash* plus `AUTH_SECRET`. So there is no session table, changing a password signs every device out, and setting `AUTH_SECRET` means a read-only database leak still cannot mint a session.
- **The 90 days slide.** A token is valid for 90 days from the sign-in that issued it, but once it is more than a week old `requireUser` reissues it on the response (`X-Session-Token`) and the client stores it. So the window is 90 days since *last use*: a device in regular use stays signed in indefinitely, and only one left idle for three months is signed out. Refreshing lives in the seam rather than in the handlers, which is why `requireUser` takes the response — no handler can forget. Agent tokens are never refreshed; they are rotated by hand.
...
- **Notes are marked as notes.** Free text — an interview note, something a recruiter said, whether typed in the drawer or posted by an agent — is stored with `kind: "note"`, which both readers skip. So a line that happens to read like `"Status: Onsite"` cannot be replayed as a stage change. `classifyEventLabel` sorts a caller-supplied label into a completion marker, a note, or a 400 (see `docs/AGENT.md`).
